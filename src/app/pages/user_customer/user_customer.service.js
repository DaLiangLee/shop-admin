        ...
        return {
            DEFAULT_GRID: {
                "columns": [{
                        ...
                        "fieldDirective": '<div class="editOrder"><span class="edit-More text-default" ng-click="propsParams.nextshow(item,$event) ">•••</span><p class="edits" ng-show="item.$show"><span class="text-primary edit-item" cb-access-control="shopapp:store:user:customer:view" add-package-dialog item="item" item-handler="propsParams.packageItem(data)">办理套餐卡</span><span class="text-primary edit-item" cb-access-control="shopapp:store:user:customer:view" charge-balance-dialog item="item" item-handler="propsParams.balanceItem(data)">充值</span><span class="text-primary edit-item" change-pwd-dialog item="item" cb-access-control="shopapp:store:user:customer:view" >修改密码</span><span class="text-primary edit-item" item="item" item-handler="propsParams.editMotorHandler(data)" add-motor-dialog cb-access-control="shopapp:store:user:customer:view" >新增车辆</span></p></div>',
                        "name": '',
                        "width": 50
                    },
                    {
                ...
                "config": {
                    ...
                    'addColumnsBarDirective': [
                        '<a class="u-btn u-btn-primary u-btn-sm" cb-access-control="shopapp:store:user:customer:add" add-new-customer item="{}" item-handler="propsParams.addCustomerHandler(data)">新增会员</a> ',
                        '<button class="u-btn u-btn-danger" cb-access-control="shopapp:store:user:customer:remove" simple-grid-remove-item="guid" item="store" remove-item="propsParams.removeItem(data)">批量删除</button> '
                    ]
                }
            },
            DEFAULT_SEARCH: {
