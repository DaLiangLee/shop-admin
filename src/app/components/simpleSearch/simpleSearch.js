            ...
            controller: ["$rootScope", "$scope", "utils", function ($rootScope, $scope, utils) {
                ...
                function setItems(config) {
                    ...
                    angular.forEach(config.searchDirective, function (item) {
                        ...
                        item.list && _.forEach(item.list, function (key) {
                            console.log(key.id, _this.searchParams[item.name])

                            key.checked = key.id+"" === _this.searchParams[item.name];
                        });
                        // 保存自定义数据
    ...
    function simpleSearchList() {
        return {
            ...
            link: function (scope, iElement, iAttrs, iCtrl) {
                ...
                scope.handler = function (data, item) {
                    _.forEach(scope.items.list, function (item) {
                        item.checked = false;
                    });
                    item.checked = true;
                    iCtrl.searchParams[scope.items.name] = data;
                }
            }
        }
    }
